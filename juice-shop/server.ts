...
restoreOverwrittenFilesWithOriginals().then(() => {
  ...
  if (process.env.NODE_ENV !== 'production') {
    whitelist.push('http://localhost:4200') // angular dev
    whitelist.push('http://127.0.0.1:4200')
    whitelist.push('http://localhost:3000')
  }

  // Options CORS réutilisables
  const corsOptions = {
    origin: (origin: string | undefined, callback: (err: Error | null, allow?: boolean) => void) => {
      // !origin permet : same-origin server requests, curl, Postman (pas d'en-tête Origin)
    ...
    credentials: true, // NE définir true qu'avec une origine explicite (pas '*')
    maxAge: 600 // cache du préflight en secondes
  }
  // Applique la config CORS globale (safe)
  ...
  // app.delete('/api/Products/:id', checkAdmin, deleteProduit) cette correction permet de verifier les privilege admin avant de supprimer un produit
